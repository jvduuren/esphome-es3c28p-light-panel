...
Print front.stl for a cable on the right, or front-usb-left.stl for one on the
left. Both have their two pry slots in the same place, on the bottom edge. The
lid fits either.

The socket is at one end of the board, so a left-hand cable means fitting the
board turned 180 degrees, and the picture and the touch have to be turned with
it. The substitutions at the top of touchpanel.yaml carry both sets of values;
swapping them is four lines.

The lid needs nothing changed: its rim is notched on both sides and the
keyholes are slotted both ways from their opening, so it hangs either way up.
Neither shows, the rim gap being internal and the keyholes facing the wall.
